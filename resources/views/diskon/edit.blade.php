                ...
                <form action="{{ route('diskon.update', $diskon->id) }}" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <select name="tipe_diskon" id="tipe_diskon" class="form-control">
                                <option value="Persen" {{ $diskon->tipe_diskon == 'Persen' ? 'selected' : '' }}>Persen (%)
                                </option>
                                <option value="Nominal" {{ $diskon->tipe_diskon == 'Nominal' ? 'selected' : '' }}>Nominal
                                    (Rp)</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div id="input_persen" class="col-md-6 form-group"
                            style="{{ $diskon->tipe_diskon == 'Persen' ? '' : 'display:none;' }}">
                            <label>Diskon Persen (%)</label>
                            <input type="number" name="diskon_persen" class="form-control"
                                value="{{ $diskon->diskon_persen }}">
                        </div>
                        <div id="input_nominal" class="col-md-6 form-group"
                            style="{{ $diskon->tipe_diskon == 'Nominal' ? '' : 'display:none;' }}">
